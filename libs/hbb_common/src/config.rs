...
lazy_static::lazy_static! {
    ...
    // BCS Beam rebrand (Brocent, 2026-08-24): customer-facing name only — the
    // "Powered by RustDesk" attribution is kept elsewhere per Jack's explicit
    // instruction (respect the upstream project), this only changes what the
    // end user sees as the app/window/tray/service name.
    pub static ref APP_NAME: RwLock<String> = RwLock::new("BCS Beam".to_owned());
...
// BCS Beam rebrand (Brocent, 2026-08-24): our self-hosted server, compiled in
// instead of relying on the runtime filename-hack pre-configuration (which
// has a documented config-overwrite-on-launch bug upstream, #13023 — baking
// it in here avoids that class of bug entirely). Source of truth for the
// actual host/key: rustdesk-test/README.md on the main ops box.
pub const RENDEZVOUS_SERVERS: &[&str] = &["beam-relay.centoffer.com"];
pub const RS_PUB_KEY: &str = "7Xy61LSDKygY5dUPtAfgUPR3i+NKLZPhpecfVesFco4=";
